#!/usr/bin/env bash
#
# evxotech-wireguard-installer-v1.3.1-02112025.sh
# EvxoTech WireGuard Installer v1.3.1 (Full reinstall + Precheck + Uninstaller)
# GitHub: https://github.com/Muneeb-Nazir/EvxoTech-Wire-Guard-Installer-
#
# Usage:
#   sudo ./evxotech-wireguard-installer-v1.3.1-02112025.sh
#   sudo ./evxotech-wireguard-installer-v1.3.1-02112025.sh --uninstall
#   sudo ./evxotech-wireguard-installer-v1.3.1-02112025.sh --uninstall --force
#
set -euo pipefail

VERSION="v1.3.1-02112025"
LOGDIR="/var/log/evxotech-wireguard"
LOGFILE="${LOGDIR}/install.log"
PRECHECK_FILENAME="evxotech-wireguard-lxc-precheck-v1.0.sh"
PRECHECK_URL_RAW="https://raw.githubusercontent.com/Muneeb-Nazir/EvxoTech-Wire-Guard-Installer-/main/${PRECHECK_FILENAME}"
PRECHECK_LOCAL="/opt/evxotech/${PRECHECK_FILENAME}"
...
# Ports (as you requested)
WG_DEFAULT_PORT="5555"
DASHBOARD_PORT_HTTP="10080"
...
require_root(){ if [ "$(id -u)" -ne 0 ]; then fatal "This script must be run as root."; fi }
...
EOF
  echo -e "${CYAN}  EvxoTech WireGuard Installer — ${VERSION}${NC}"
  echo
}
...
if [[ "${1:-}" == "--uninstall" ]]; then
  ...
  # run uninstaller immediately
  echo
  echo -e "${YELLOW}==========================================${NC}"
  echo -e "${YELLOW}   EvxoTech WireGuard Uninstaller         ${NC}"
  echo -e "${YELLOW}==========================================${NC}"
  if [ "$FORCE_UNINSTALL" != "true" ]; then
    read -rp $'\e[33mAre you sure you want to remove WireGuard and the EvxoTech Dashboard? (y/N): \e[0m' CONFIRM
    if [[ ! "$CONFIRM" =~ ^[Yy]$ ]]; then
  ...
  systemctl stop evxotech-wg-dashboard-https.service 2>/dev/null || true
  systemctl disable evxotech-wg-dashboard-https.service 2>/dev/null || true
  ...
  rm -f /etc/systemd/system/evxotech-wg-dashboard.service /etc/systemd/system/evxotech-wg-dashboard-https.service 2>/dev/null || true
  rm -rf "$LOGDIR" 2>/dev/null || true

  ok "Uninstall complete. Backups (if any) are left under /etc/wireguard-backup-*"
  exit 0
fi
...
version_check() {
  ...
  if [ -n "$latest" ] && [ "$latest" != "$VERSION" ]; then
    ...
    ok "Installer: ${VERSION}"
  fi
}

download_precheck() {
  mkdir -p /opt/evxotech
  if [ -f "$PRECHECK_LOCAL" ]; then
    info "Precheck script already at ${PRECHECK_LOCAL}"
    chmod +x "$PRECHECK_LOCAL"
    return 0
  fi
  info "Downloading precheck script from GitHub..."
  if curl -fsSLo "$PRECHECK_LOCAL" "$PRECHECK_URL_RAW"; then
    chmod +x "$PRECHECK_LOCAL"
    ok "Downloaded precheck to ${PRECHECK_LOCAL}"
    return 0
  else
    warn "Could not download precheck from ${PRECHECK_URL_RAW}. You can place precheck at ${PRECHECK_LOCAL} manually."
    return 1
  fi
}

run_precheck_check() {
  if [ ! -x "$PRECHECK_LOCAL" ]; then fatal "Precheck script missing or not executable: ${PRECHECK_LOCAL}"; fi
  info "Running precheck (--check)..."
  if bash "$PRECHECK_LOCAL" --check --ctid "${CTID:-}" ; then
    ok "Precheck: OK"
    return 0
  else
...
run_precheck_fix() {
  info "Running precheck (--fix --yes)..."
  if bash "$PRECHECK_LOCAL" --fix --ctid "${CTID:-}" --yes ; then
    ok "Precheck fixes applied"
    return 0
  else
    fatal "Precheck automatic fix failed. Inspect ${PRECHECK_LOCAL} logs."
  fi
}
...
prompt_user() {
  ...
  echo
  read -rp "If you want the installer to patch the Proxmox host LXC config, enter the CTID (or leave blank if installing inside the container already): " CTID
  read -rp "Public IP or Domain for clients to connect [${PUB_DETECT}]: " PUBLIC_IP
  PUBLIC_IP=${PUBLIC_IP:-$PUB_DETECT}
  read -rp "Server LAN IP (dashboard host) [${LOCAL_DETECT}]: " SERVER_LAN_IP
  SERVER_LAN_IP=${SERVER_LAN_IP:-$LOCAL_DETECT}
  read -rp "WireGuard listening port [${WG_DEFAULT_PORT}]: " WG_PORT
  WG_PORT=${WG_PORT:-$WG_DEFAULT_PORT}
  read -rp "WireGuard internal CIDR [10.120.80.0/24]: " LAN_CIDR
  LAN_CIDR=${LAN_CIDR:-10.120.80.0/24}
  read -rp "Number of client configs to create initially [1]: " CLIENT_COUNT
  CLIENT_COUNT=${CLIENT_COUNT:-1}
  read -rp "DNS to push to clients [10.120.80.10]: " DNS_PLACEHOLDER
  DNS_PLACEHOLDER=${DNS_PLACEHOLDER:-10.120.80.10}
  echo -e "Enter admin password for the dashboard (press Enter for default: ${ADMIN_PASS_DEFAULT}):"
  ...
  echo " Dashboard (HTTP): ${SERVER_LAN_IP}:${DASHBOARD_PORT_HTTP}"
  read -rp "Proceed with these settings? (y/N): " CONF
  if [[ ! "$CONF" =~ ^[Yy]$ ]]; then
...
install_packages() {
  ...
  apt-get install -y build-essential linux-headers-$(uname -r) wireguard wireguard-dkms wireguard-tools qrencode python3-venv python3-pip iproute2 iptables curl wget git >/dev/null 2>&1 || {
    fatal "Failed to install packages. See $LOGFILE"
  }
...
EOF
  ...
  ok "Server config written to ${SERVER_CONF} (WG IP: ${WG_SERVER_IP})"
}
...
WGADD
  ...
  sed -e "s|__PUBLIC_IP__|${PUBLIC_IP:-127.0.0.1}|g" \
      -e "s|__WG_PORT__|${WG_PORT:-5555}|g" \
      -e "s|__LAN_CIDR__|${LAN_CIDR:-10.120.80.0/24}|g" \
      -e "s|__DNS__|${DNS_PLACEHOLDER:-10.120.80.10}|g" \
  ...
  ok "Installed wg-add-client helper"
}

bring_up_wg_with_fallback() {
  info "Bringing up WireGuard interface ${WG_IF}..."
  if wg-quick up "${WG_IF}" >/dev/null 2>&1; then
    ok "wg-quick up succeeded"
    return 0
  fi
  warn "wg-quick failed; attempting manual fallback (ip + wg setconf)..."
  ip link delete dev "${WG_IF}" >/dev/null 2>&1 || true
  ip link add dev "${WG_IF}" type wireguard >/dev/null 2>&1 || fatal "Failed to add wireguard link: kernel module missing?"
...
setup_dashboard() {
  ok "Installing minimal Flask dashboard at ${DASHBOARD_DIR}..."
...
SERVICE
  ...
  systemctl start evxotech-wg-dashboard.service >/dev/null 2>&1 || warn "Dashboard service failed to start; check systemctl status."
  ok "Dashboard installed (HTTP ${DASHBOARD_PORT_HTTP})."
}

offer_https() {
  read -rp "Do you want to enable self-signed HTTPS for dashboard on port ${DASHBOARD_PORT_HTTPS}? (y/N): " CH
  if [[ "$CH" =~ ^[Yy]$ ]]; then
    ...
    cat > /etc/systemd/system/evxotech-wg-dashboard-https.service <<SVC
[Unit]
...
SVC
    ...
    systemctl daemon-reload >/dev/null 2>&1
    systemctl enable evxotech-wg-dashboard-https.service >/dev/null 2>&1
    systemctl start evxotech-wg-dashboard-https.service >/dev/null 2>&1 || warn "HTTPS dashboard service failed to start; check systemctl status."
    ok "HTTPS dashboard started on ${DASHBOARD_PORT_HTTPS} (self-signed)."
  else
    warn "Skipping HTTPS setup."
  fi
}

create_uninstaller_helper() {
  cat > /usr/local/bin/evxotech-wg-uninstall <<'UNINST'
#!/usr/bin/env bash
echo "EvxoTech WireGuard Uninstall helper"
read -rp "Type YES to confirm uninstall: " CONF
...
UNINST
  ...
  ok "Created /usr/local/bin/evxotech-wg-uninstall"
}
...
final_summary() {
  ...
  echo "WG internal address: ${WG_SERVER_IP}/24"
  echo "Client configs & QR: ${CLIENT_DIR}"
  echo "Dashboard (HTTP): http://${SERVER_LAN_IP}:${DASHBOARD_PORT_HTTP}"
  if systemctl is-active --quiet evxotech-wg-dashboard-https.service 2>/dev/null; then
    echo "Dashboard (HTTPS): https://${SERVER_LAN_IP}:${DASHBOARD_PORT_HTTPS} (self-signed)"
  fi
  echo "Admin: ${ADMIN_USER} (password as provided during install)"
  echo
...
main() {
  ...
  # download precheck
  if ! download_precheck; then
    warn "Precheck download failed — you can place ${PRECHECK_FILENAME} at ${PRECHECK_LOCAL} and re-run."
  fi

  # run precheck in check-only mode
  read -rp "Run precheck now? (Y/n): " RUN_PRECHECK
  RUN_PRECHECK=${RUN_PRECHECK:-Y}
  if [[ "$RUN_PRECHECK" =~ ^[Yy]$ ]]; then
    if run_precheck_check; then
      ok "Precheck OK — continuing."
    else
      read -rp "Precheck found issues. Attempt automatic fixes? (y/N): " WANT_FIX
      if [[ "$WANT_FIX" =~ ^[Yy]$ ]]; then
        read -rp "Enter CTID of container to patch (required for fix): " CTID
        if [ -z "${CTID}" ]; then fatal "CTID is required for auto-fix."; fi
        export CTID
        run_precheck_fix
      else
  ...
  # write server public key inline for convenience
  echo "# Server public key:" >> "${SERVER_CONF}"
  cat /etc/wireguard/server_public.key >> "${SERVER_CONF}"

  if ! bring_up_wg_with_fallback; then
    fatal "WireGuard interface failed to start. Check ${LOGFILE} and ${SERVER_CONF}"
  fi
